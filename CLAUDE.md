...
- `EtaWorker` — 15-minute `PeriodicWorkRequest`; restarts the service with `ACTION_RESUME` only if `prefs.isTracking()` and the service isn't in `getRunningServices()`.
...
- **ETA trend state is deliberately in-memory only.** `EtaForegroundService.lastPollEta` supplies `EXTRA_PREV_ETA`; it is never persisted, and `StatusActivity` passes `-1` on resume so no stale cross-session trend is shown. Do not "fix" this by moving it into `AppPreferences`: a `KEY_PREV_ETA` pref existed once and was deliberately removed, because it made a new session open showing a trend computed against the *previous* destination.
...
- **`ACTION_START` vs `ACTION_RESUME`.** Only `ACTION_START` (user pressing Start in `SetupActivity`) resets `saveTrackingStartTime`, `savePollCount` and `saveLastEta`. `ACTION_RESUME` — sent by `EtaWorker`, and used for the null-intent `START_STICKY` restart — keeps the original start time, and bails out immediately if the window has already expired. Reviving a session with `ACTION_START` renews the tracking window forever; that bug shipped once.
- The service is `START_STICKY`, so the system can re-create it with a null intent. That path resumes if `prefs.isTracking()` and otherwise calls `stopTracking()` — it must never fall through, or `onCreate()`'s unconditional 3-hour wake lock is held by a service that does nothing. `EtaWorker` is still the primary reviver of a live session, since `onDestroy()` writes `saveTracking(false)`.
...
- The "things that will bite you" notes above encode past regressions, not style preferences. Each one describes a bug that shipped; treat them as constraints when changing polling, duration, or trend logic.
